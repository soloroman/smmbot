  ...
  data: {
    ...
    messages: [
      ...
      },
      {
        user: 0,
        text:
          "This example is a demonstration of [Rete.js](https://github.com/retejsrete) framework. Based on it you can make your powerful editor"
      },
      {
        user: 0,
        text:
...
var datSocket = new Rete.Socket("Data");
...
class CheckControl extends Rete.Control {
  ...
  handler(el, editor) {
    ...
    var option = this.getData(this.key) || this.options[0];

    check.selectedIndex = 0;
    this.putData(this.key, option);
    check.addEventListener("change", () => {
      this.putData(this.key, this.options[check.selectedIndex]);
    });
  }
}
...
class SelectControl extends Rete.Control {
  ...
  handler(el, editor) {
    ...
    select.addEventListener("change", () => {
      this.putData(this.key, this.options[select.selectedIndex]);
    });
  }
}

class PushEventComponent extends Rete.Component {
  constructor() {
    super("Push event");
    this.task = {
      outputs: { act: "option", data: "output" },
      init(task) {
  ...
  builder(node) {
    var out1 = new Rete.Output("act", "Action", actSocket, true);
    var out2 = new Rete.Output("data", "Data", datSocket, true);
    var ctrl2 = new CheckControl("option", ['A', 'B', 'C']);
    return node.addOutput(out1).addOutput(out2).addControl(ctrl2);
  }
...
class MessageEventComponent extends Rete.Component {
  ...
  builder(node) {
    var out1 = new Rete.Output("act", "Action", actSocket, true);
    var out2 = new Rete.Output("text", "Text", strSocket, true);
